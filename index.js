...
const dPage = handlebars.compile(fs.readFileSync(`${__dirname}/views/partials/default_page.handlebars`)
	.toString('utf-8'))
handlebars.registerPartial('default_page', dPage)
const postLoginHeader = handlebars.compile(fs.readFileSync(`${__dirname}/views/partials/loggedInHeader.handlebars`)
	.toString('utf-8'))
...
router.get('/item/:id', async ctx => {
	try {
		...
		await ctx.render( 'listing',
			Object.assign({}, data, { authorised: ctx.session.authorised, ownListings: listingData})
...
router.post('/item/:id', async ctx => {
	try {
		if (ctx.session.authorised !== true) throw new Error('Only logged in users can send offers.')
		...
		if (listingInfo.lister_id === ctx.session.user_id) throw new Error('Cannot make an offer on an item you own')

		const user = await new User(dbName)

		const listerInfo = await user.getUserDataFromID(listingInfo.lister_id)

		const msg = `Hello ${listerInfo.username},\nThe user ${ctx.session.username} has offered a trade for one \
        of your item listings!\n\nThey wish to trade their '${usersListings[ctx.request.body.swapitem]}' for your\
         '${listingInfo.itemname}'.\n\nYour item listing: http://localhost:8080/item/${ctx.params.id}\n\nYou can \
         reach them at ${ctx.session.email} in order to discuss this trade further.\n\nHave a great day,\nExchangeBay.`
...
router.get('/login', async ctx => {
	...
	await ctx.render('login', Object.assign({}, data, { authorised: ctx.session.authorised }))
})
...
router.get('/accountPage', async ctx => {
	try {
		...
		return ctx.render('accountPage',
			{ authorised: ctx.session.authorised, user_id: ctx.session.user_id, username: ctx.session.username,
				forename: ctx.session.forename, surname: ctx.session.surname, email: ctx.session.email }
		)
